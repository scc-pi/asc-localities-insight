...
if (!is.na(measure_type)){
  if (measure_type == "percentage"){
    plot <- plot +
        geom_text_repel(data = subpage_data, force = 0, size = 3,
                        bg.color = "white", bg.r = 0.15,
                        aes(X, Y, 
                            label = str_c(asc_name, 
                                          "\n",
                                          percent(.data[[measure_item]],
                                                  accuracy = 0.1, 
                                                  scale = 1)))) +
    ...
    plot <- plot +
      geom_text_repel(data = subpage_data, force = 0, 
                      bg.color = "white", bg.r = 0.15, size = 3,
                      aes(X, Y, 
                          label = str_c(asc_name, 
                                        "\n",
                                        comma(.data[[measure_item]], 
                                              accuracy = 1)))) +
        scale_fill_viridis(discrete = FALSE,
                     name = measure_item,
                     direction = -1,
